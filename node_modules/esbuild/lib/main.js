...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.8.3") {
        throw new Error(`Cannot start service: Host version "${"0.8.3"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
  ...
  let handlePlugins = (plugins, request, buildKey) => {
    ...
    for (let item of plugins) {
      ...
      if (typeof setup !== "function")
        throw new Error(`[${plugin.name}] Missing a setup function`);
      i++;
      setup({
        onResolve(options, callback) {
          ...
          checkForInvalidFlags(options, keys);
          if (filter == null)
            throw new Error(`[${plugin.name}] "onResolve" is missing a filter`);
        ...
        onLoad(options, callback) {
          ...
          checkForInvalidFlags(options, keys);
          if (filter == null)
            throw new Error(`[${plugin.name}] "onLoad" is missing a filter`);
          let id = nextCallbackID++;
          ...
          for (let id of request2.ids) {
            try {
              ...
              if (result != null) {
                ...
                if (errors != null)
                  response.errors = sanitizeMessages(errors);
                if (warnings != null)
    ...
    service: {
      build(options, isTTY2, callback) {
        ...
        try {
          ...
          let [flags, write, plugins, stdin, resolveDir] = flagsForBuildOptions(options, isTTY2, logLevelDefault);
          let request = {command: "build", key, flags, write, stdin, resolveDir};
...
function sanitizeMessages(messages) {
  ...
  for (const message of messages) {
    ...
    if (location != null) {
      ...
      checkForInvalidFlags(location, keys2);
      locationClone = {
        file: file || "",
        namespace: namespace || "",
        line: line || 0,
...
let startService = (options) => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.8.3"}`), {
    cwd: process.cwd(),
    windowsHide: true,
...
let runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.8.3"}`), {
    cwd: process.cwd(),
    windowsHide: true,
    input: stdin,
